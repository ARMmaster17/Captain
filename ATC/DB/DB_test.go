...
func TestConnectToDB(t *testing.T) {
	db, err := ConnectToDB()
	if err != nil {
		t.Errorf("unexpected error in creating Sqlite3 database:\n%w", err)
	}
	if db == nil {
...
func TestDBDefaultConnectionString(t *testing.T) {
	...
	if err != nil {
		t.Errorf("unexpected error getting default connection string:\n%w", err)
		return
	}
...
func TestGetConfiguredDBDriverEnv(t *testing.T) {
	...
	if err != nil {
		t.Errorf("unexpected error connnecting to database:\n%w", err)
	}
	if db.Name() != "sqlite" {
